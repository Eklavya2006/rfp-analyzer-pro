...
// Uses pdf-parse (Node.js only) so it never runs in the browser.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Read into a Node.js Buffer — this is the only place pdf-parse is called
    const arrayBuffer = await file.arrayBuffer();
    const buffer = Buffer.from(arrayBuffer);

    // Dynamically import pdf-parse (Node.js only — safe here on the server)
    ...
    const pageCount = parsed.numpages > 0 ? parsed.numpages : Math.max(1, Math.round(text.split(/\s+/).length / 300));
