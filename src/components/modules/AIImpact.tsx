...
import EmailComposerModal, { type EmailSection } from '@/components/EmailComposerModal';
...
export default function AgenticImpactModule() {
  ...
  function emailReport() { setEmailOpen(true); }

  const emailSections: EmailSection[] = [
    // ── Narrative (cover message) always first ─────────────────
    {
      title: 'Narrative',
      type: 'text',
      text: `This AI Impact Report demonstrates a ${ai.overallProductivityGain}% overall productivity gain through AI-augmented delivery across ${ai.roleRows.length} roles and ${ai.phaseRows.length} project phases. Total hours saved: ${ai.totalHoursSaved.toLocaleString()}h.\n\nGenerated by IBM RFP Analyzer Pro.`,
    },
    {
      title: 'AI Impact Summary',
      type: 'table',
      rows: [
        { label: 'Document',            value: docName,                                       highlight: true },
        { label: 'Generated',           value: new Date().toLocaleString() },
        { label: 'Active Agents',       value: ai.roleRows.length,                            badge: 'Agents', badgeColor: '#0f62fe' },
        { label: 'Total AI Hours',      value: `${ai.totalAIHours.toLocaleString()}h`,         highlight: true },
        { label: 'Hours Saved',         value: `${ai.totalHoursSaved.toLocaleString()}h`,      badge: 'Saved', badgeColor: '#059669' },
        { label: 'Traditional Hours',   value: `${ai.totalTraditionalHours.toLocaleString()}h` },
        { label: 'Productivity Gain',   value: `${ai.overallProductivityGain}%`,               highlight: true, badge: 'Gain', badgeColor: '#7c3aed' },
      ],
    },
    {
      ...
      type: 'table',
      collapsible: true,
      rows: ai.roleRows.map(r => ({
        label: `${r.role} (${r.band})`,
        value: `${r.traditionalFTE} FTE → ${r.aiAugmentedFTE} FTE`,
        badge: `${r.productivityPct}% prod`,
        badgeColor: '#0f62fe',
      })),
    },
    {
      ...
      type: 'table',
      collapsible: true,
      rows: ai.phaseRows.map(p => ({
        label: p.phase,
        value: `${p.traditionalHours}h → ${p.aiAssistedHours}h`,
        badge: `${p.hoursSaved}h saved`,
        badgeColor: '#059669',
      })),
    },
  ];
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={emailReport}
            className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg border transition-all hover:bg-blue-100"
            style={{ borderColor: '#0f62fe', color: '#0f62fe', background: '#eff6ff' }}
          >
            <Mail size={13} />
            Email Report
          </button>
      ...
      <EmailComposerModal
        ...
        defaultTo="pradeep.lamba1@ibm.com"
        subject={`AI Impact Report — ${docName}`}
        reportTitle="AI Impact Report"
        subtitle={docName}
        avatarInitials="AI"
        avatarColor="#0f62fe"
