...
// ── Email share helper — direct mailto: (IBM SMTP auth disabled by tenant) ──
function shareViaEmail(clientName: string, projectTitle: string) {
  const subject = encodeURIComponent(`RFP Proposal — ${projectTitle} for ${clientName}`);
  const body    = encodeURIComponent(
    `Hi,\n\nPlease find attached the IBM proposal for ${projectTitle}.\n\nGenerated by RFP Analyzer Pro.\n\nBest regards`
  );
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                  onClick={() => shareViaEmail(
                    result?.scopeItems?.[0]?.description?.split(' ')[0] ?? 'Client',
                    doc?.summary?.title ?? 'Enterprise Project',
                  )}
                  ...
                  title="Opens your default mail client with proposal details pre-filled"
                >
