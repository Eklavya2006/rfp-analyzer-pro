...
import { FileText, Download, Link2, Image, Globe, Wand2, Mail } from 'lucide-react';
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: '#E2E8F0' }}>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold border transition-colors hover:bg-blue-50"
                  style={{ borderColor: '#0F62FE', color: '#0F62FE', background: 'transparent' }}
                >
      ...
      {(() => {
        ...
        const emailSections: EmailSection[] = [
          {
            ...
            type: 'table',
            rows: [
              { label: 'Client',            value: clientName,                                                highlight: true },
              { label: 'Project',           value: projectTitle,                                              highlight: true },
              { label: 'Date',              value: date },
              { label: 'Scope Items',       value: result?.scopeItems?.filter(s => s.category === 'in-scope').length ?? 0,  badge: 'In-Scope', badgeColor: '#0f62fe' },
              { label: 'Deliverables',      value: result?.deliverableItems?.length ?? 0 },
              { label: 'Project Phases',    value: plan?.phases.length ?? 0 },
              { label: 'Duration',          value: plan ? `${plan.totalDurationWeeks} weeks` : '—' },
              { label: 'Engagement Value',  value: est ? fmt(est.adjustedTotalCost ?? est.totalCost) : '—', highlight: true, badge: 'USD', badgeColor: '#059669' },
            ],
          },
          ...(staff ? [{
            title: 'Staffing Plan',
            type: 'table' as const,
            collapsible: true,
            rows: staff.roles.map(r => ({
              label: r.roleName,
              value: `${r.numberOfResources} × ${r.hoursPerResource}h @ $${r.hourlyRate}/hr`,
              badge: r.band,
              badgeColor: '#7c3aed',
            })),
          }] : []),
        ...
        return (
          <EmailComposerModal
            ...
            subject={`IBM Proposal — ${projectTitle} for ${clientName}`}
            reportTitle={`Proposal: ${projectTitle}`}
            subtitle={`${clientName} · ${date}`}
            avatarInitials="PL"
            avatarColor="#1e3a5f"
